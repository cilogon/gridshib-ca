...
sub _authorize
{
    ...
    if (!defined($1))
    {
        $self->_error("Authorization failed. Could not extract hostname " .
                      "from peer DN (%d).", $peerDN);
        return 0;
    }
    my $peerHostname = $1;
...
sub _getpeername()
{
    ...
    if (inet_ntoa($peerAddr) eq inet_ntoa(INADDR_LOOPBACK))
    {
        ...
        if (!defined($peername))
        {
            $self->_error("Authorization failed. Could not resolve " .
                          "local hostname.");
            return undef;
        }
    }
